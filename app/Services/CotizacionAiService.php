...
class CotizacionAiService
{
     ...
     * - items mapeados al catálogo
     * - alternativas y pendientes_ai
     * Params:
    ...
    public function aiParse(Request $r)
    {
        ...
        try {
            ...
            /* =====================================================================
               NUEVO (C) DETECTOR/PARSER DETERMINÍSTICO PARA TABLAS TIPO "ANEXO UNO"
               Estructura detectada en tu PDF:
               PARTIDA  CLAVE(10 dígitos)  DESCRIPCIÓN ...  CANTIDAD  UNIDAD
               (ver páginas del anexo). Si lo detecta, saca TODO sin IA.
               ===================================================================== */
            @file_put_contents($logStepFile, '['.date('c')."] Paso 6C: intentar parseAnnexTableAllPages()\n", FILE_APPEND);
            $annexItemsAll = $this->parseAnnexTableAllPages($pages);
            if (count($annexItemsAll) >= 20) {
                $reason = 'Detectada tabla tipo ANEXO/REQUISICIÓN; extracción determinística sobre todas las páginas.';
            }
            @file_put_contents($logStepFile, '['.date('c')."] Paso 6C OK: annex_items=".count($annexItemsAll)."\n", FILE_APPEND);

            // Paso 7: extracción IA (solo si no detectó tabla)
            @file_put_contents($logStepFile, '['.date('c')."] Paso 7: extracción IA\n", FILE_APPEND);
            $parsed = [];
            if (count($annexItemsAll) >= 20) {
                // Ya tenemos items confiables: usamos IA solo para licitación/resumen si hay tiempo
                if (!$this->timeUp($deadline)) {
...
Reglas: Devuelve SOLO JSON válido.
...
PR);
                ...
                $parsed['items'] = $annexItemsAll; // forzamos items 100% determinísticos
...
PR);
            ...
            // Paso 7B: refuerzo local + fusión (solo si NO fue annex)
            if (count($annexItemsAll) < 20) {
                ...
                foreach (($parsed['items'] ?? []) as $it) {
                    ...
                    $byKey[$k] = [
                        ...
                        'clave_verificacion' => $it['clave_verificacion'] ?? null,
                    ];
                }
                ...
                if (!$alts) {
                    $pendientes[] = [
                        'raw' => [
                            ...
                            'clave_verificacion' => $row['clave_verificacion'] ?? null,
    ...
    private function extractPdfPagesText(string $path): array
    {
        ...
        if ($ocrKey) {
            try {
                ...
                if ($wasOk && trim($textFromApi) !== '') return [[ $textFromApi ], true];
            } catch (\Throwable $e) {
    ...
    private function ocrSpace(string $pdfPath, string $apiKey): array
    {
        ...
        if ($stream === false) throw new \RuntimeException("No se pudo abrir el PDF para OCR: $pdfPath");
    ...
    private function productSearchQuery(string $queryText)
    {
        ...
        foreach ($tokens as $t) {
            $qb->where(function($w) use ($cols, $t) {
                foreach ($cols as $c) $w->orWhere($c, 'LIKE', "%{$t}%");
            });
        }
    ...
    /* ======================= NUEVOS HELPERS PARA TABLAS ANEXO ======================= */

    /**
     * Intenta extraer partidas de tablas tipo:
     * "PARTIDA CLAVE DE VERIFICACIÓN ... CANTIDAD UNIDAD"
     * Devuelve items con: partida, clave_verificacion, nombre, descripcion, cantidad, unidad
     */
    private function parseAnnexTableAllPages(array $pagesText): array
    {
        $lines = [];
        foreach ($pagesText as $txt) {
            $txt = preg_replace("/\r\n|\r/u", "\n", (string)$txt);
            foreach (explode("\n", $txt) as $ln) {
                $ln = trim($ln);
                if ($ln !== '') $lines[] = $ln;
        ...
        $startRx = '/^\s*(\d{1,3})\s+(\d{10})\s+(.+)$/u';

        foreach ($lines as $ln) {
            // ignora encabezados
            if (preg_match('/^\s*PARTIDA\b/i', $ln)) continue;
            if (preg_match('/^\s*CLAVE\b/i', $ln)) continue;
            if (preg_match('/^\s*DIRECCI[ÓO]N\b/i', $ln)) continue;

            if (preg_match($startRx, $ln, $m)) {
                if ($current) $items[] = $this->finalizeAnnexRow($current);
        ...
        // limpia vacíos
        $items = array_values(array_filter($items, fn($it)=>!empty($it['descripcion'])));

        // orden por partida
        usort($items, fn($a,$b)=>($a['partida'] ?? 0) <=> ($b['partida'] ?? 0));

        return $items;
    }

    private function finalizeAnnexRow(array $cur): array
    {
        $text = preg_replace('~\s+~u', ' ', trim((string)($cur['desc'] ?? '')));

        $units = '(PIEZA|PIEZAS|PAQUETE|CAJA|BOLSA|JUEGO|JGO|ROLLO|BLISTER|KIT|MTS?|CM|M|LITRO|LITROS|MILILITROS?|GRAMOS?)';
        $qty = 1;
        $unit = 'PIEZA';

        if (preg_match('/\b(\d[\d,\.]*)\s+'.$units.'\s*$/iu', $text, $m)) {
            $qty = $this->parseNumber($m[1]);
            $unit = strtoupper($m[2]);

            // quita "qty + unit" del final para dejar solo descripción
            $text = trim(preg_replace('/\b'.preg_quote($m[1],'/').'\s+'.$units.'\s*$/iu', '', $text));
        }

        if ($qty <= 0) $qty = 1;

        return [
            'partida'            => (int)($cur['partida'] ?? 0),
            'clave_verificacion' => $cur['clave'] ?? null,
            'nombre'             => $text,
            'descripcion'        => $text,
            'cantidad'           => $qty,
            'unidad'             => $unit,
