...
class VehicleController extends Controller
{
  ...
  /**
   * ✅ NECESARIO porque tu index manda a /vehicles/{id}
  ...
  public function show(Request $request, Vehicle $vehicle)
  {
    ...
    // si viene desde fetch con Accept: application/json
    if ($request->expectsJson() || $request->wantsJson()) {
  ...
  public function store(Request $request)
  {
    ...
    return redirect()->route('vehicles.index')->with('ok', 'Camioneta creada');
  }
  ...
  public function update(Request $request, Vehicle $vehicle)
  {
    ...
    return redirect()->route('vehicles.edit', $vehicle)->with('ok', 'Actualizado');
  }
  ...
  public function destroy(Request $request, Vehicle $vehicle)
  {
    ...
    // borra agenda events vinculados
    ...
    return $request->expectsJson() || $request->wantsJson()
      ...
      : redirect()->route('vehicles.index')->with('ok', 'Eliminado');
  }
  ...
  private function validated(Request $request, ?int $ignoreId = null): array
  {
    ...
    return $request->validate([
      ...
      // ⚠️ “sin restricción” real depende de php.ini (upload_max_filesize/post_max_size)
  ...
  private function syncAgendaDates(Vehicle $vehicle): void
  {
    ...
    $vehicle->agenda_verification_id = $this->upsertAgendaEvent(
      $vehicle->agenda_verification_id,
      $vehicle->next_verification_due_at,
      "Verificación · {$vehicle->plate}",
      "Próxima verificación de {$vehicle->brand} {$vehicle->model} {$vehicle->year}",
      $userId,
      $tz
    );

    $vehicle->agenda_service_id = $this->upsertAgendaEvent(
      $vehicle->agenda_service_id,
      $vehicle->next_service_due_at,
      "Servicio · {$vehicle->plate}",
      "Próximo servicio de {$vehicle->brand} {$vehicle->model} {$vehicle->year}",
      $userId,
      $tz
    );

    $vehicle->agenda_tenencia_id = $this->upsertAgendaEvent(
      $vehicle->agenda_tenencia_id,
      $vehicle->tenencia_due_at,
      "Tenencia · {$vehicle->plate}",
      "Vencimiento de tenencia de {$vehicle->plate}",
      $userId,
      $tz
    );

    $vehicle->agenda_circulation_id = $this->upsertAgendaEvent(
      $vehicle->agenda_circulation_id,
      $vehicle->circulation_card_due_at,
      "Tarjeta Circulación · {$vehicle->plate}",
      "Vencimiento de tarjeta de circulación de {$vehicle->plate}",
      $userId,
      $tz
    );
    ...
    if (!is_null($vehicle->insurance_due_at)) {
      $vehicle->agenda_insurance_id = $this->upsertAgendaEvent(
        $vehicle->agenda_insurance_id,
        $vehicle->insurance_due_at,
        "Seguro · {$vehicle->plate}",
        "Vencimiento de seguro de {$vehicle->plate}",
        $userId,
        $tz
      );
    } else {
      // si quedó sin fecha, limpia evento
      $vehicle->agenda_insurance_id = $this->upsertAgendaEvent(
        $vehicle->agenda_insurance_id,
        null,
        '',
  ...
  private function upsertAgendaEvent(?int $agendaId, $date, string $title, string $desc, int $userId, string $tz): ?int
  {
    ...
    $event->remind_offset_minutes = 1440; // 1 día antes
