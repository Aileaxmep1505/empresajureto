...
class AgendaEventController extends Controller
{
    ...
    public function store(Request $r)
    {
        ...
        // Convertir la entrada (que viene del datetime-local input) a string "Y-m-d H:i:s"
        $data['start_at'] = $this->datetimeLocalToDbString($data['start_at'], $tz);

        $event = new AgendaEvent($data);
        $event->computeNextReminder(); // compute usando la zona del evento y guardará next_reminder_at naive
        $event->save();
