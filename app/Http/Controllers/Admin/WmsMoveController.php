...
class WmsMoveController extends Controller
{
    ...
    public function products(Request $r)
    {
        ...
        $bestLocByItem = Inventory::query()
            ...
            ->whereIn(
                'location_id',
                Location::query()->where('warehouse_id', $warehouseId)->pluck('id')
            )
        ...
        foreach ($lines as $ln) {
            if (!empty($ln['location_id']) && !isset($whLocSet[(int) $ln['location_id']])) {
                return response()->json([
                    'ok' => false,
                    'error' => 'Una ubicación no pertenece a esta bodega.',
                ], 422);
            }
        }
            ...
            $result = DB::transaction(function () use (
                ...
                foreach ($lines as $ln) {
                    ...
                    if (!$locationId && $it->primary_location_id) {
                        ...
                        if (Location::query()->where('warehouse_id', $warehouseId)->where('id', $pl)->exists()) {
                            $locationId = $pl;
                        }
                    }
                    ...
                    if (!$locationId) {
                        ...
                        if ($best) {
                            $locationId = (int) $best->location_id;
                        }
                    }
    ...
    public function movementsData(Request $r)
    {
        ...
        $query = WmsMovementLine::query()
            ->whereHas('movement', function ($m) use ($warehouseId, $type, $from, $to) {
                ...
                if ($from) {
                    $m->whereDate('created_at', '>=', $from);
                }
                if ($to) {
                    $m->whereDate('created_at', '<=', $to);
                }
            })
            ->with([
