...
class PropuestaComercialMatchController extends Controller
{
    ...
    protected function generateExternalReferences(PropuestaComercialItem $item, string $descripcion, string $unidad): void
    {
        ...
        $externalResults = $this->externalReferenceService->searchTop3(
    ...
    protected function rankProduct(
        ...
        foreach ($allWords as $word) {
            ...
            if ($this->wordIn($name, $word)) {
                $ws += 20;
            }

            if ($this->wordIn($category, $word)) {
                $ws += 15;
            }

            if ($this->wordIn($tags, $word)) {
                $ws += 14;
            }

            if ($this->wordIn($brand, $word)) {
                $ws += 8;
            }

            if ($this->wordIn($description, $word)) {
                $ws += 6;
            }

            if ($this->wordIn($sku, $word)) {
                $ws += 10;
            }
