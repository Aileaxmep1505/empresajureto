(() => {
  ...
  const CLIENTES_INFO   = Object.fromEntries(Object.entries(BOOT.clientesInfo || {}));
  ...
  const btnParseHandler = async (e)=>{
    e.preventDefault();
    if(!ROUTES.aiParse){ alert('No se definió la ruta de análisis.'); return; }
    ...
    try{
      ...
      document.getElementById('ai_f_pub').textContent = F.publicacion ?? '—';
      document.getElementById('ai_f_acl').textContent = F.aclaraciones ?? '—';
      document.getElementById('ai_f_pre').textContent = F.presentacion ?? '—';
      document.getElementById('ai_f_fal').textContent = F.fallo ?? '—';
  ...
  if (btnParse) btnParse.addEventListener('click', btnParseHandler);
