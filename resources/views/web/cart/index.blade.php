...
@php
  ...
  $resolveImg = function($raw){
    ...
    // si viene como "catalog/photos/..." lo convierte a "/storage/catalog/photos/.."
...
<style>
  ...
  #cart .top{display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;margin-bottom:24px;}
  ...
  #cart .pill{display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:999px;background:var(--blue-soft);color:var(--blue);font-weight:700;font-size:14px;white-space:nowrap;}
  ...
  #cart .card{background:var(--card);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden;}
  #cart .card-body{padding:24px;}
  #cart .sticky{position:sticky;top:24px;}
  #cart .btn{appearance:none;border:1px solid transparent;border-radius:999px;padding:12px 20px;font-weight:700;font-size:14px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:8px;text-decoration:none;transition:all .2s ease;user-select:none;width:100%;}
  #cart .btn-primary{background:var(--blue);color:#fff;box-shadow:0 4px 12px rgba(0,122,255,0.2);}
  #cart .btn-ghost{background:#fff;border-color:var(--blue);color:var(--blue);}
  #cart .btn-danger{background:var(--danger-soft);color:var(--danger);width:auto;padding:8px 16px;font-size:13px;}
  #cart .help-link{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:8px;font-weight:600;font-size:13px;color:var(--muted);text-decoration:none;width:100%;transition:color .2s ease;margin-top:8px;}
  #cart .help-link:hover{color:var(--blue);}
  #cart .table{width:100%;border-collapse:collapse;}
  #cart .table th,#cart .table td{padding:16px 10px;border-bottom:1px solid var(--line);vertical-align:middle;}
  #cart .table thead th{font-size:11px;color:var(--muted);font-weight:700;text-transform:uppercase;letter-spacing:1px;background:#fff;border-bottom:2px solid var(--line);}
  #cart .row{display:flex;align-items:center;gap:16px;}
  #cart .thumb{width:70px;height:70px;border-radius:12px;object-fit:contain;border:1px solid var(--line);background:#fff;padding:4px;}
  #cart .prod-name{display:inline-block;font-weight:600;font-size:15px;color:#111;text-decoration:none;line-height:1.3;transition:color .2s;}
  #cart .sku{color:var(--muted);font-weight:500;margin-top:4px;font-size:12px;}
  #cart .qty{display:inline-flex;align-items:center;border:1px solid var(--line);border-radius:var(--radius-sm);overflow:hidden;background:#fff;}
  #cart .qty button{border:0;background:transparent;padding:8px 12px;min-width:36px;font-size:16px;font-weight:600;cursor:pointer;color:var(--muted);}
  #cart .qty input{width:40px;text-align:center;border:0;border-left:1px solid var(--line);border-right:1px solid var(--line);height:36px;outline:0;font-family:inherit;font-weight:600;color:var(--ink);background:transparent;}
  #cart .hr{border:0;border-top:1px solid var(--line);margin:20px 0;}
  #cart .kv{display:flex;justify-content:space-between;align-items:center;font-weight:600;font-size:15px;color:#444;}
  #cart .kv .k{color:var(--muted);font-weight:500;}
  #cart .total{font-weight:800;font-size:22px;color:#111;}
  #cart .freebox{border:1px solid #cce0ff;border-radius:12px;background:var(--blue-soft);padding:16px;margin-bottom:24px;}
  #cart .freebox__title{font-weight:700;color:var(--blue);display:flex;align-items:center;gap:8px;font-size:15px;margin-bottom:6px;}
  #cart .freebox__text{color:#555;font-size:13px;font-weight:500;line-height:1.4;}
  #cart .progress{height:6px;background:#fff;border-radius:999px;overflow:hidden;margin-top:12px;}
  #cart .progress > div{height:100%;width:0%;background:var(--blue);border-radius:999px;transition:width .4s ease;}
  @media(max-width:760px){
    #cart .table thead{display:none;}
    #cart .table tr{display:flex;flex-direction:column;gap:12px;padding:16px 0;border-bottom:1px solid var(--line);}
      ...
      <div class="card col-main">
        @if(count($cart ?? []) === 0)
          <div class="card-body">
            <div style="padding:60px 20px;text-align:center;color:var(--muted);">
              <h2 style="color:#111;font-size:24px;margin:0;font-weight:700;">Tu carrito está vacío</h2>
              <p style="margin:10px 0 0;font-size:15px;">Explora nuestro catálogo y descubre productos increíbles.</p>
              <a class="btn btn-primary" style="width:auto;padding:14px 32px;margin-top:20px;" href="{{ route('web.catalog.index') }}">
                Explorar catálogo
              </a>
            </div>
          </div>
        @else
                  ...
                  <tr data-id="{{ $row['id'] }}">
                    ...
                    <td class="cell-qty" data-label="Cantidad" style="text-align:center;">
                      <div class="qty">
                        ...
                        <input type="number" min="1" max="999" value="{{ (int)$row['qty'] }}"
                               onchange="cartSet({{ $row['id'] }}, this.value)">
                        <button type="button" aria-label="Aumentar" onclick="cartPlus({{ $row['id'] }})">+</button>
                      </div>
                    </td>

                    <td class="row-total" data-label="Importe" style="text-align:right; font-weight:700; color: var(--blue);">
          ...
          <div class="card-body" style="border-top:1px solid var(--line);display:flex;gap:16px;justify-content:space-between;flex-wrap:wrap;align-items:center;background:#fafafa;">
            <a class="btn btn-ghost" style="width:auto;" href="{{ route('web.catalog.index') }}">Seguir comprando</a>
          ...
          <div class="card-body">
            ...
            @if($FREE_SHIP > 0)
              <div id="freeboxWrapper">
                ...
                @else
                  <div style="padding:12px;border-radius:12px;border:1px solid #bbf7d0;background:#e6ffe6;color:#15803d;font-weight:700;text-align:center;">
            ...
            <div style="margin-top:24px;">
              ...
              <a class="help-link" href="{{ route('web.contacto') }}">¿Necesitas ayuda? Contáctanos</a>
            </div>
          </div>
        </div>
      </aside>
