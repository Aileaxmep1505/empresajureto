...
/* Ocultar tema y categor√≠a visualmente */
#helpdesk .new .grid{
              ...
              @foreach($savedTickets as $saved)
                ...
                <div class="saved-chat-row {{ (int)$activeTicketId === (int)$saved->id ? 'active' : '' }}">
                  ...
                  <form class="saved-chat-delete-form"
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="saved-chat-delete" title="Borrar chat">√ó</button>
                  </form>
                </div>
              @endforeach
            </div>
        ...
        @unless($ticket)
          <form class="new" id="newTicketForm" action="{{ route('help.start') }}" method="POST">
            ...
            <div style="margin-top:12px; display:flex; gap:10px; flex-wrap:wrap; align-items:center">
              <button class="btn" type="submit">Enviar al asistente IA ‚ú®</button>
  ...
  if (newForm) {
    newForm.addEventListener('submit', async (ev) => {
      ...
      try {
        ...
        if (btn) { btn.disabled = false; btn.textContent = 'Enviar al asistente IA ‚ú®'; }
      }
    });
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (textarea) {
    textarea.placeholder = 'Escribe tu problema o pregunta. Ejemplo: No puedo completar mi pago, ya revis√© mi direcci√≥n y el env√≠o aparece correcto...';
  }
});
</script>
...
<style>
/* Bot√≥n √∫nico para agrandar/reducir caja de texto */
...
/* =========================================================
   ...
   Visual only. No toca lÛgica IA / carrito / tickets.
========================================================= */
...
<style>
/* FIX REAL: barra #composerInput con tamaÒo actual y contorno luminoso */
...
<style>
/* Movimiento visible de la lÌnea iluminada del composer */
#helpdesk #composerInput {
  background:
    linear-gradient(135deg, #07111f, #0b1628) padding-box,
    conic-gradient(
...
<style>
/* FIX: permitir agrandar caja sin perder lÌnea animada */
...
/* TamaÒo normal cuando NO est· expandida */
...
<style>
/* =====================================================
   JURETO: lÌnea animada en chats pasados y botones
===================================================== */
...
/* BotÛn fantasma: conservar blanco pero con lÌnea animada */
#helpdesk .btn-ghost,
#helpdesk #escalarBtn {
  color: #2563eb !important;
  background:
...
<style>
/* FIX: que + Nuevo y agrandar caja sigan viÈndose como botones */
#helpdesk #expandComposerBtn,
#helpdesk .mini-expand {
...
#helpdesk input[placeholder*="QuÈ quieres encontrar"],
#helpdesk input[placeholder*="quieres encontrar"],
input[placeholder*="QuÈ quieres encontrar"],
...
/* Evitar que el efecto de botones afecte el botÛn AI del buscador */
#helpdesk input[placeholder*="QuÈ quieres encontrar"] ~ button,
#helpdesk input[placeholder*="quieres encontrar"] ~ button,
input[placeholder*="QuÈ quieres encontrar"] ~ button,
...
/* Si el contenedor del buscador recibiÛ borde animado, apagarlo */
#helpdesk input[placeholder*="QuÈ quieres encontrar"]::before,
#helpdesk input[placeholder*="QuÈ quieres encontrar"]::after,
...
#helpdesk input[placeholder*="QuÈ quieres encontrar"]:parent,
#helpdesk input[placeholder*="quieres encontrar"]:parent {
  background: #ffffff !important;
}

/* Apagar efectos en el input y en su botÛn AI */
#helpdesk input[placeholder*="QuÈ quieres encontrar"],
#helpdesk input[placeholder*="QuÈ quieres encontrar"] + *,
#helpdesk input[placeholder*="quieres encontrar"],
#helpdesk input[placeholder*="quieres encontrar"] + * {
...
/* BotÛn AI del buscador como antes */
#helpdesk input[placeholder*="QuÈ quieres encontrar"] ~ button,
#helpdesk input[placeholder*="quieres encontrar"] ~ button,
input[placeholder*="QuÈ quieres encontrar"] ~ button,
input[placeholder*="quieres encontrar"] ~ button {
...
#helpdesk form:has(input[placeholder*="QuÈ quieres encontrar"]),
#helpdesk div:has(> input[placeholder*="QuÈ quieres encontrar"]),
#helpdesk div:has(input[placeholder*="QuÈ quieres encontrar"]) {
  background: #ffffff !important;
  border: 1px solid rgba(15,23,42,.08) !important;
  border-radius: 999px !important;
...
document.addEventListener('DOMContentLoaded', function () {
  function restoreSearchBar() {
    const input = Array.from(document.querySelectorAll('input')).find(function (el) {
      return (el.getAttribute('placeholder') || '').toLowerCase().includes('quÈ quieres encontrar') ||
...
/* SeparaciÛn general para que respiren */
#helpdesk .chat {
...
/* PequeÒa sombra bajo cada burbuja */
#helpdesk .msg {
...
/* Al enfocar, lÌnea luminosa suave */
...
<style>
/* TEST VISIBLE: confirmar que la vista sÌ est· actualizando */
#helpdesk {
...
/* 4) BotÛn secundario claro y limpio */
#helpdesk #escalarBtn,
#helpdesk .btn-ghost {
  background:
...
/* 5) BotÛn agrandar caja claro, sin deformarse */
#helpdesk #expandComposerBtn,
#helpdesk .mini-expand {
  background:
    linear-gradient(135deg, rgba(255,255,255,.98), rgba(242,248,255,.94)) padding-box,
