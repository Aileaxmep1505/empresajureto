...
@php
  /*
   * Vista de edición basada en la vista de crear.
   * Requiere que el controlador mande:
   * $routePlan cargado con stops y driver
   * $drivers
   * $providers
   */
  $selectedDriverId = (int) old('driver_id', $routePlan->driver_id);
  $routeName = old('name', $routePlan->name);

  $oldStopsRaw = old('stops');
  $initialPickedStops = [];

  if (is_string($oldStopsRaw) && trim($oldStopsRaw) !== '') {
      $decoded = json_decode($oldStopsRaw, true);
      if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
          $initialPickedStops = $decoded;
      }
  } elseif (is_array($oldStopsRaw)) {
      $initialPickedStops = $oldStopsRaw;
  }

  if (empty($initialPickedStops)) {
      $initialPickedStops = $routePlan->stops
          ->sortBy(function($stop){
              return sprintf('%06d-%06d', $stop->sequence_index ?? 999999, $stop->id);
          })
          ->values()
          ->map(function($stop){
              return [
                  'id' => $stop->id,
                  'provider_id' => $stop->provider_id ?? null,
                  'name' => $stop->name,
  ...
  $existingProviderIds = collect($initialPickedStops)
      ->pluck('provider_id')
      ->filter()
      ->map(fn($id) => (int) $id)
      ->values()
...
<div id="rp-create">
  ...
  <div class="wrap">
    @if($errors->any())
      <div class="alert alert-warning slide-left">
    ...
    <div class="pagehead slide-left">
      <div>
        ...
        <p class="subtitle">Actualiza el chofer, agrega o elimina puntos y vuelve a calcular con Google Maps.</p>
      </div>

      <a href="{{ route('routes.show', $routePlan) }}" class="back-btn">
        <i class="bi bi-arrow-left"></i> Volver
      </a>
    </div>

    <form id="routeForm" method="POST" action="{{ route('routes.update', $routePlan) }}" class="grid-layout">
      @csrf
      @method('PUT')
        ...
        <div class="cardx">
          ...
          <div class="bd">
            <div class="field">
              ...
              <select name="driver_id" class="control" required>
                ...
                @foreach($drivers as $d)
                  <option value="{{ $d->id }}" @selected($selectedDriverId === (int) $d->id)>
            ...
            <div class="field">
              ...
              <input type="text" name="name" class="control" placeholder="Ej. Ruta Zona Norte – 1" value="{{ $routeName }}">
            </div>
                ...
                @forelse($providers as $p)
                  ...
                  <label class="prov-item">
                    <input class="custom-check provChk" type="checkbox"
                           @checked(in_array((int) $p->id, $existingProviderIds, true))
                           data-id="{{ $p->id }}"
...
const picked = @json($initialPickedStops);
...
document.getElementById('routeForm').addEventListener('submit', (e) => {
  ...
  if (!picked.length) {
    ...
    valAlert.innerHTML = '<i class="bi bi-exclamation-triangle-fill" style="margin-right:8px;"></i> Debes dejar al menos un punto en la ruta.';
    window.scrollTo({
