...
<body>
  @php
    ...
    // Datos de asignación (ajusta los nombres de campo a tu esquema si difieren)
    $isAssigned = $item->type === 'activo_fijo' && $item->asset_status === 'asignado';
    $assignedTo = $item->assigned_to
      ?? $item->assigned_user_name
      ?? optional($item->assignedUser ?? null)->name
      ?? optional($item->responsible ?? null)->name
      ?? null;
    $assignedAtRaw = $item->assigned_at ?? $item->assignment_date ?? null;
    $assignedAt = $assignedAtRaw ? \Carbon\Carbon::parse($assignedAtRaw)->format('d/m/Y') : null;
  @endphp
