...
@push('styles')
<style>
/* ====== ENCAPSULADO GENERAL DE LA PÁGINA DE USUARIOS ====== */
.users-page *{
  /* Forzamos escritura horizontal dentro de esta vista,
     por si hay algún writing-mode global del proyecto */
...
.users-page .hero__left{
  display:flex;
  align-items:center;
  gap:12px;
  min-width:280px;
}
.users-page .hero__icon{
  width:48px;
  height:48px;
  border-radius:50%;
  display:grid;
...
@media (max-width: 576px){
  .users-page .hero__icon{ display:none }
}
...
.users-page .sb-clear:hover{
  background:#f1f5f9;
  color:#64748b;
}

@media (max-width:768px){
  .users-page .hero{ padding:14px }
  .users-page .hero__right{
    width:100%;
...
.users-page .card:hover{
  ...
  box-shadow:
    ...
    0 0 0 4px rgba(59,130,246,.08) inset;
}
...
/* Nombre del usuario SIEMPRE horizontal */
.users-page .u-name{
  margin:0;
  line-height:1.2;
...
.users-page .btn:hover{ filter:brightness(1.02) }
.users-page .btn:active{ transform:translateY(1px) }
.users-page .btn.primary{
  background:linear-gradient(135deg,#7EA8FF,#A8C9FF);
  color:#0f1f47;
  border-color:#9bbcfb;
}
.users-page .btn.danger{
  background:linear-gradient(135deg,#ff9bbd,#ff6a9a);
  color:#4a0f23;
  border-color:#ff92b9;
}
...
/* ====== Select estilizado ====== */
.users-page select.input,
.users-page .input-select{
  appearance:none;
...
.users-page select.input:focus,
.users-page .input-select:focus{
  outline:none;
  border-color:#9bbcfb;
  box-shadow:0 0 0 6px rgba(91,141,239,.12);
}
...
.users-page .badge.pending{
  background:#fff5d9;
  color:#8a6d1a;
  border-color:#f5e6b5;
  position:relative;
}
.users-page .badge.approved{
  background:#dcfce7;
  color:#14532d;
  border-color:#bbf7d0;
}
.users-page .badge.revoked{
  background:#ffe4e6;
  color:#7f1d1d;
  border-color:#fecdd3;
}
.users-page .badge.pending::after{
  content:"";
  position:absolute;
  inset:-2px;
  border-radius:999px;
...
.users-page .pagination .hidden{ display:none }
.users-page .pagination nav{
  display:flex;
  gap:6px;
  flex-wrap:wrap;
}
.users-page .pagination a,
.users-page .pagination span{
  padding:10px 12px;
  border-radius:10px;
  border:1px solid var(--border);
  background:var(--surface);
...
.users-page .pagination a:hover{ background:#eef3ff }
.users-page .pagination .active span{
  background:#e3ebff;
  color:var(--primary-600);
}

/* ====== Toast ====== */
.users-page .toast-area{
  position:fixed;
  right:14px;
  top:14px;
  display:flex;
...
.users-page .toast{
  ...
  opacity:0;
  animation:toastIn .45s cubic-bezier(.2,.9,.2,1) forwards;
  box-shadow:0 10px 30px rgba(30,46,90,.12);
}
.users-page .toast--ok{ border-color:#a7f3d0 }
.users-page .toast--err{ border-color:#fecdd3 }
.users-page .toast__icon{
  width:22px;
  height:22px;
  display:grid;
  place-items:center;
}
.users-page .toast__text{ font-weight:700 }
.users-page .toast__sub{
  color:var(--muted);
  font-weight:600;
  font-size:.88rem;
}
...
.users-page .swal-rounded.swal2-popup{
  border-radius:18px !important;
  padding:18px !important;
}
.users-page .swal-confirm{
  border-radius:12px;
  padding:10px 14px;
  font-weight:800;
  background:linear-gradient(135deg,#ff9bbd,#ff6a9a);
...
.users-page .swal-confirm:hover{ filter:brightness(1.04) }
.users-page .swal-cancel{
  border-radius:12px;
  padding:10px 14px;
  font-weight:800;
  background:#fff;
...
.users-page .swal-cancel:hover{ background:#f8fafc }
</style>
@endpush
    ...
    @if (session('status'))
      <div class="toast-area" id="toastArea">
        <div class="toast toast--ok">
          ...
          <div>
            <div class="toast__text">{{ session('status') }}</div>
            <div class="toast__sub">Listo</div>
          </div>
        </div>
      </div>
    @endif
...
(function(){
  ...
  document.querySelectorAll('.card.user').forEach((el,i)=>{
    el.style.animation=`heroIn .45s ease ${i*70}ms both`;
  });
})();
</script>
@endpush
