    ...
    <div class="pj-toolbar">
        ...
        <div class="pj-toolbar-right">
                ...
                <div class="pj-popover pj-filter-pop" id="filter-pop">
                    <div class="pj-filter-scroll">
                        ...
                        <div class="pj-filter-section">
                            ...
                            <div class="pj-check-list">
                                <label class="pj-check-row"><span class="pj-pri-label"><i class="pj-pri-dot is-red"></i> Alta</span><input type="checkbox"><span class="pj-square"></span></label>
                                <label class="pj-check-row"><span class="pj-pri-label"><i class="pj-pri-dot is-orange"></i> Media</span><input type="checkbox"><span class="pj-square"></span></label>
                                <label class="pj-check-row"><span class="pj-pri-label"><i class="pj-pri-dot is-green"></i> Baja</span><input type="checkbox"><span class="pj-square"></span></label>
                                <label class="pj-check-row"><span class="pj-pri-label"><i class="pj-pri-dot is-gray"></i> Normal</span><input type="checkbox"><span class="pj-square"></span></label>
                            </div>
                        </div>
                        <div class="pj-filter-section">
                            <div class="pj-filter-title">Rango de fechas</div>
                            <div class="pj-date-grid">
                                <div class="pj-date-input"><input type="text" placeholder="dd/mm/aaaa"><svg viewBox="0 0 24 24" fill="none" class="pj-date-icon"><path d="M7 3v3M17 3v3M4 9h16M6 5h12a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg></div>
                                <div class="pj-date-input"><input type="text" placeholder="dd/mm/aaaa"><svg viewBox="0 0 24 24" fill="none" class="pj-date-icon"><path d="M7 3v3M17 3v3M4 9h16M6 5h12a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg></div>
                            </div>
                        </div>
                    </div>
            ...
            <button type="button" class="pj-btn pj-btn-light pj-btn-icon-only" title="Eliminar">
                <svg viewBox="0 0 24 24" fill="none" class="pj-icon"><path d="M4 7h16M9 7V5h6v2m-8 0 1 12h8l1-12" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
                                ...
                                @if($column['count'] > 0)
                                    @foreach($column['projects'] as $index => $project)
                                        ...
                                        <div class="pj-item-row js-project-row"
                                            ...
                                            <div class="pj-col pj-col-priority"><span class="pj-priority is-normal">{{ $priority }}</span></div>
                                            ...
                                            <div class="pj-card js-project-row"
                                                ...
                                                <div class="pj-card-meta">
                                                    ...
                                                    <div class="pj-meta-row">
                                                        ...
                                                        <span class="pj-priority @if(($project['priority'] ?? 'Normal') === 'Alta') is-high @elseif(($project['priority'] ?? 'Normal') === 'Baja') is-low @else is-normal @endif">
...
<div class="pj-bulkbar" id="pjBulkbar" aria-hidden="true">
    <div class="pj-bulkbar-inner">
        ...
        <div class="pj-bulkbar-actions">
            <button type="button" class="pj-bulk-action" id="pjBulkLabelsBtn">Etiquetas</button>
            <button type="button" class="pj-bulk-action">Favoritos</button>
            <button type="button" class="pj-bulk-action">Color</button>
            <button type="button" class="pj-bulk-action">Mover a etapa</button>
            <button type="button" class="pj-bulk-action">Prioridad</button>
            <button type="button" class="pj-bulk-action">Asignar</button>
            <button type="button" class="pj-bulk-action">Archivar</button>
            <button type="button" class="pj-bulk-action is-danger">Eliminar</button>
        </div>
    </div>
</div>
...
<div class="pj-project-menu" id="pjProjectMenu" aria-hidden="true">
    <div class="pj-project-menu-card">
        <button type="button" class="pj-project-menu-item">Cambiar nombre</button>
        <button type="button" class="pj-project-menu-item">Asignar usuario</button>
        <button type="button" class="pj-project-menu-item">Mover a otra etapa</button>
        <button type="button" class="pj-project-menu-item">Quitar de favoritos</button>
        <button type="button" class="pj-project-menu-item">Editar etiquetas</button>
        <button type="button" class="pj-project-menu-item">Editar prioridad</button>
        <button type="button" class="pj-project-menu-item">Copiar link</button>
        <button type="button" class="pj-project-menu-item">Cambiar color</button>
        <div class="pj-project-menu-divider"></div>
        <button type="button" class="pj-project-menu-item">Archivar</button>
        <button type="button" class="pj-project-menu-item is-danger">Eliminar</button>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            if (!noDocs && projectFiles.length === 0) {
                alert('Selecciona al menos un archivo o marca "Crear proyecto sin documentos".');
                return;
            }
    ...
    function renderLabelOptions(filter = '') {
        ...
        labelOptions.innerHTML = filtered.map(item => `<button type="button" class="pj-label-option" data-label="${item.name}">${item.name}</button>`).join('');
    }
    ...
    function openLabelPopover(anchor, mode = 'single') {
        activeLabelTarget = anchor; labelMode = mode;
        placeFloating(labelPopover, anchor, 8);
        labelPopover.classList.add('is-open'); labelPopover.setAttribute('aria-hidden', 'false');
        labelSearchInput.value = ''; createLabelText.textContent = 'Etiqueta';
        renderLabelOptions('');
    ...
    function openProjectMenu(anchor) { activeProjectAnchor = anchor; placeFloating(projectMenu, anchor, 8); projectMenu.classList.add('is-open'); projectMenu.setAttribute('aria-hidden', 'false'); }
    function closeProjectMenu() { activeProjectAnchor = null; projectMenu.classList.remove('is-open'); projectMenu.setAttribute('aria-hidden', 'true'); }
    document.querySelectorAll('.js-open-project-menu').forEach(btn => btn.addEventListener('click', function (e) { e.stopPropagation(); openProjectMenu(this); }));
    document.addEventListener('click', function (e) {
        ...
        if (!e.target.closest('.js-open-project-menu') && !e.target.closest('#pjProjectMenu')) closeProjectMenu();
    });

    window.addEventListener('resize', () => {
        if (activeLabelTarget && labelPopover.classList.contains('is-open')) placeFloating(labelPopover, activeLabelTarget, 8);
