...
<style>
  ...
  #expensesPage .hero:hover { box-shadow: 0 15px 50px -10px rgba(15, 23, 42, 0.12); }
  ...
  #expensesPage .card:hover { box-shadow: 0 15px 50px -10px rgba(15, 23, 42, 0.12); }
  ...
  #expensesPage .form-control:focus, #expensesPage .form-select:focus{
  ...
  #expensesPage .xcard:hover{ transform: translateY(-3px); box-shadow: 0 15px 45px -10px rgba(15, 23, 42, 0.1); border-color: var(--pblue-strong); }
  #expensesPage .thumb{
  ...
  #expensesPage .title{ font-weight:800; letter-spacing:-.01em; font-size:16px; color: #0f172a; }
  ...
  #expensesPage .tab-pill.active{ background:var(--text); color:#fff; box-shadow:0 8px 20px rgba(15,23,42,.2); transform: translateY(-1px); }
      ...
      <div class="hero mt-2 mb-4">
        ...
        <div class="mobile-switch">
          <div class="d-flex gap-2">
            ...
            <a class="btn btn-pastel-green" href="{{ route('expenses.create') }}"><i class="bi bi-plus-lg"></i></a>
          </div>
        </div>
      </div>
        ...
        <div class="tab-pane fade show active" id="pane-dash" role="tabpanel" tabindex="0">
          ...
          <div class="card mt-4 animate-fade-in" style="animation-delay: 0.5s;">
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-3">
              <div>
                <span class="fs-5"><i class="bi bi-bar-chart-fill me-2 text-primary"></i>Tendencia de gastos</span>
                <div class="small fw-bold text-primary mt-1" id="chartRangeHint">Mostrando: Últimos 14 días</div>
              </div>
            ...
            <div class="card-body p-0">
              <div class="table-responsive">
                <table class="table table-hover mb-0 align-middle">
                  ...
                  <tbody id="tbody">
                    <tr><td colspan="7" class="p-4"><div class="skeleton mb-2" style="height:20px;"></div></td></tr>
                  </tbody>
                </table>
              </div>
            </div>
        ...
        <div class="tab-pane fade" id="pane-list" role="tabpanel" tabindex="0">
          <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3 bg-white p-3 rounded-4 border shadow-sm animate-fade-in">
            ...
            <div class="d-flex gap-3 align-items-center">
              ...
              <div class="btn-group">
                <button class="btn btn-outline-secondary btn-sm px-3 rounded-start-pill" id="btnPrev" type="button"><i class="bi bi-chevron-left"></i></button>
                <button class="btn btn-outline-secondary btn-sm px-3 rounded-end-pill" id="btnNext" type="button"><i class="bi bi-chevron-right"></i></button>
              </div>
            </div>
          </div>
          <div id="grid" class="grid"><div class="empty animate-fade-in">Cargando movimientos...</div></div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer border-0 d-flex justify-content-between">
        <div class="d-flex gap-2 flex-wrap">
          <a class="btn btn-pastel-blue fw-bold d-none px-4" id="evDownload" href="#" target="_blank" rel="noopener"><i class="bi bi-cloud-download-fill me-2"></i> Descargar</a>
          <a class="btn btn-pastel-green fw-bold d-none px-4" id="rcDownload" href="#" target="_blank" rel="noopener"><i class="bi bi-file-earmark-pdf-fill me-2"></i> Ver PDF</a>
        </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header border-0 pb-0">
        <div><div class="fw-bold fs-5" id="stTitle">Modificar Estatus</div><div class="text-muted small mt-1" id="stSub">—</div></div>
      ...
      <div class="modal-footer border-0 pt-0">
        ...
        <button class="btn btn-pastel-blue rounded-pill px-4" type="button" id="stSave"><i class="bi bi-check2-circle me-1"></i> Guardar Cambios</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade expenses-scope" id="deleteModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-danger">
      ...
      <div class="modal-footer border-0 pt-0">
        <button class="btn btn-outline-secondary rounded-pill px-4" data-bs-dismiss="modal" type="button">Cancelar</button>
        <button class="btn btn-danger rounded-pill px-4 fw-bold" style="box-shadow: 0 4px 15px rgba(220,38,38,.3);" type="button" id="delConfirm">Eliminar definitivamente</button>
      </div>
    </div>
  </div>
</div>
...
(() => {
  ...
  let state = { page: 1, last_page: 1, per_page: 20, total: 0, rows: [], timer: null, lastChartRows: null, chart_group: 'day' };
  ...
  function pct(part, total){ return total <= 0 ? 0 : Math.round((Number(part||0) / Number(total||1)) * 1000) / 10; }
  ...
  function toast(type, message, opts = {}){
    const container = $('expensesToasts'); if(!container) return;
    const id = 't_' + Math.random().toString(16).slice(2);
    const icon = (type === 'success') ? 'bi-check-circle-fill text-success' : (type === 'danger')  ? 'bi-x-circle-fill text-danger' : 'bi-info-circle-fill text-primary';
    container.insertAdjacentHTML('beforeend', `<div id="${id}" class="toast bg-white text-dark border-0 animate-fade-in shadow-lg" role="alert" aria-live="assertive" aria-atomic="true" data-bs-delay="${opts.delay ?? 3000}"><div class="d-flex align-items-center p-2"><div class="toast-body d-flex align-items-center fs-6"><i class="bi ${icon} me-3 fs-4"></i><span>${esc(message)}</span></div><button type="button" class="btn-close me-3 m-auto" data-bs-dismiss="toast"></button></div></div>`);
    const el = $(id); const t = new bootstrap.Toast(el); el.addEventListener('hidden.bs.toast', ()=> el.remove()); t.show();
  }

  function setMiniError(id, show, msg){
    const el = $(id); if(!el) return;
    if(show){ el.classList.remove('d-none'); el.innerHTML = `<i class="bi bi-exclamation-triangle-fill me-2"></i> ${esc(msg || 'Error')}`; } else{ el.classList.add('d-none'); el.textContent = ''; }
  }
  ...
  async function apiUpdateStatus(id, status){
    const res = await fetch(endpointFromTemplate(UPDATE_URL_TPL, id), { method: 'PATCH', headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken(), 'X-Requested-With': 'XMLHttpRequest' }, body: JSON.stringify({ status }) });
    if(!res.ok) throw new Error(`Error al actualizar (HTTP ${res.status}).`); return res.json().catch(()=> ({}));
  }
  async function apiDeleteExpense(id){
    const res = await fetch(endpointFromTemplate(DESTROY_URL_TPL, id), { method: 'DELETE', headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrfToken(), 'X-Requested-With': 'XMLHttpRequest' } });
    if(!res.ok) throw new Error(`Error al eliminar (HTTP ${res.status}).`); return res.json().catch(()=> ({}));
  }

  function normalizeDate(d){ const s = String(d||'').slice(0,10); return (/^\d{4}-\d{2}-\d{2}$/.test(s) && !s.startsWith('0000')) ? s : null; }
  function fmtDate(d){
    const s = normalizeDate(d); if(!s) return 'Fecha no registrada';
    const dt = new Date(s + 'T00:00:00'); return dt.toLocaleDateString('es-MX', {day:'2-digit', month:'short', year:'numeric'});
  }

  function warn(show, msg){ const w = $('warn'); if(!w) return; show ? (w.classList.remove('d-none'), w.innerHTML = `<i class="bi bi-wifi-off me-2"></i> ${esc(msg)}`) : w.classList.add('d-none'); }

  function params(extra = {}){
    const p = new URLSearchParams();
    const map = {q:'q', cat:'category_id', veh:'vehicle_id', status:'status', from:'from', to:'to'};
    for(const [id, key] of Object.entries(map)){ const val = $(id)?.value?.trim(); if(val) p.set(key, val); }
    p.set('per_page', String(state.per_page)); p.set('page', String(state.page)); p.set('_t', String(Date.now()));
  ...
  const chart = chartEl ? new Chart(chartEl, {
    ...
    options:{
      ...
      scales:{ x:{ stacked:true, grid:{ display:false } }, y:{ stacked:true, border:{dash: [4, 4]}, grid:{ color:'rgba(148,163,184,.15)' } } }
    }
  }) : null;
  ...
  const pie = pieEl ? new Chart(pieEl, {
    type: 'doughnut',
    data: { labels: [], datasets: [{ data: [], backgroundColor: ['#60a5fa', '#34d399', '#fbbf24', '#f472b6', '#0ea5e9', '#94a3b8'], borderWidth: 2, borderColor: '#ffffff', hoverOffset: 4 }]},
    options: {
      responsive:true, maintainAspectRatio:false, cutout: '70%',
      plugins: {
        legend: { position: 'bottom', labels:{ usePointStyle:true, padding: 20 } },
        tooltip: { backgroundColor: 'rgba(15, 23, 42, 0.9)', padding: 12, cornerRadius: 8, callbacks:{ label: (ctx)=>{ const val = Number(ctx.parsed||0); const total = ctx.dataset.data.reduce((a,b)=>a+Number(b||0),0) || 1; return ` ${ctx.label}: ${money(val)} (${Math.round((val/total)*100)}%)`; } } }
      }
    }
  }) : null;

  function setTrend7FromChartRows(rows, currency='MXN'){
    const el = $('trend7'); if(!el) return;
    ...
    const last = values[values.length - 1] || 0; const prev = values.length > 1 ? (values[values.length - 2] || 0) : 0;
    let arrow = '<i class="bi bi-arrow-right text-muted"></i>';
    if(prev > 0){ const diffPct = ((last - prev) / prev) * 100; arrow = diffPct > 0 ? '<i class="bi bi-arrow-up-right text-danger"></i>' : (diffPct < 0 ? '<i class="bi bi-arrow-down-right text-success"></i>' : arrow); } else if (last > 0) arrow = '<i class="bi bi-arrow-up-right text-danger"></i>';
  ...
  async function loadMetrics(){
    ...
    if(!res.ok){ warn(true, 'Error API métricas.'); return null; }
    const data = await res.json().catch(()=>null); if(!data) return null; warn(false);

    const currency = data.currency ?? 'MXN'; const paid = Number(data.paid_sum ?? 0); const pending = Number(data.pending_sum ?? 0); const canceled = Number(data.canceled_sum ?? 0);
    ...
    $('kPaid').textContent = money(paid, currency); $('kPending').textContent = money(pending, currency); $('kCanceled').textContent = money(canceled, currency); $('kTotal').textContent = money(total, currency);
    $('kCount').textContent = String(data.count ?? 0); $('kTotalHint').innerHTML = total > 0 ? '<i class="bi bi-check-circle-fill me-1"></i> Totales calculados' : 'Sin registros para este filtro';

    if($('kPaidPct')) $('kPaidPct').textContent = `${pct(paid, total)}% del total`;
    if($('kPendingPct')) $('kPendingPct').textContent = `${pct(pending, total)}% del total`;
    if($('kCanceledPct')) $('kCanceledPct').textContent = `${pct(canceled, total)}% del total`;
    $('kpiCount').textContent = String(data.count ?? 0); $('kpiSum').textContent = money(total, currency);
  ...
  async function loadChart(){
    ...
    chart.data.datasets[2].data = rows.map(r => r.canceled);
    chart.update();
  }
  ...
  function openEvidence(e){
    ...
    const pushField = (label, value) => {
      ...
      fields.push(`<div class="col-12 col-md-6 col-lg-4"><div class="border rounded-4 p-3 h-100 bg-light"><div class="small text-muted mb-1 fw-bold">${esc(label)}</div><div class="fw-black text-dark" style="word-break:break-word; font-size:1.05rem;">${esc(String(value))}</div></div></div>`);
    };

    pushField('ID Registro', e.id); pushField('Fecha de Pago', fmtDate(e.expense_date)); pushField('Monto Total', money(e.amount, e.currency || 'MXN'));
    pushField('Concepto', e.concept); pushField('Descripción', (e.description || '').trim()); pushField('Categoría', e.category?.name); pushField('Vehículo', e.vehicle?.plate); pushField('Método', e.payment_method);
  ...
  function openEditStatus(e){ if(!statusModalInst) statusModalInst = new bootstrap.Modal($('statusModal')); $('stId').value = e.id; $('stSub').textContent = `${esc(e.concept)} • ${esc(money(e.amount))}`; $('stStatus').value = (String(e.status).toLowerCase() === 'cancelled') ? 'canceled' : String(e.status||'paid').toLowerCase(); setMiniError('stErr', false); statusModalInst.show(); }
  function openDelete(e){ if(!deleteModalInst) deleteModalInst = new bootstrap.Modal($('deleteModal')); $('delId').value = e.id; $('delTitle').textContent = e.concept; $('delSub').textContent = `${fmtDate(e.expense_date)} • ${money(e.amount)}`; setMiniError('delErr', false); deleteModalInst.show(); }

  $('stSave')?.addEventListener('click', async ()=>{ try{ await apiUpdateStatus($('stId').value, $('stStatus').value); statusModalInst.hide(); toast('success', `Estatus actualizado.`); refreshAll(); } catch(err){ setMiniError('stErr', true, err.message); } });
  $('delConfirm')?.addEventListener('click', async ()=>{ try{ await apiDeleteExpense($('delId').value); deleteModalInst.hide(); toast('success', `Registro eliminado.`); refreshAll(); } catch(err){ setMiniError('delErr', true, err.message); } });

  function renderCards(){
    const grid = $('grid');
    if(!state.rows.length){ grid.innerHTML = `<div class="empty animate-fade-in"><i class="bi bi-inbox fs-1 d-block mb-3 opacity-50"></i>No se encontraron movimientos.</div>`; return; }

    grid.innerHTML = state.rows.map((e, index) => {
      ...
      const statusText = isPaid ? 'Pagado' : statusLabel(e.status);

      const chips = [ e.status ? `<span class="tag border-0 bg-light"><i class="bi bi-circle-fill small ${statusClass}"></i> ${esc(statusText)}</span>` : '' ].filter(Boolean).join('');

      return `
        <div class="xcard animate-fade-in" style="animation-delay: ${index * 0.05}s;">
          <div class="thumb"><i class="bi bi-wallet2 fs-2"></i><div class="small">${esc(date.split(' ')[0])}</div></div>
          <div class="flex-grow-1" style="min-width:0">
            <div class="d-flex justify-content-between gap-2 align-items-start">
              <div class="title text-truncate" title="${esc(concept)}">${esc(concept)}</div>
              <div class="amount px-2 py-1 bg-light rounded-3 border">${esc(amount)}</div>
            </div>
            ${desc ? `<div class="small text-muted mt-1 text-truncate" style="max-width: 100%;">${esc(desc)}</div>` : ''}
  ...
  function buildInsightsFromRows(rows){
    ...
    const top6 = [...dMap.entries()].sort((a,b)=>b[1]-a[1]); const labels = top6.slice(0,5).map(x=>x[0]); const values = top6.slice(0,5).map(x=>x[1]);
    const others = top6.slice(5).reduce((a,b)=>a+b[1],0); if(others>0){ labels.push('Otros'); values.push(others); }
