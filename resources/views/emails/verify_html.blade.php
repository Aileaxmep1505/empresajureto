@php
  ...
  $ink      = '#0f172a';
  $line     = '#e6e8ef';
  $pre      = 'Confirma tu correo para activar tu cuenta en '.config('app.name').'.';
@endphp
<!doctype html>
<html>
<head>
        ...
        <tr><td style="background:#fff;border:1px solid {{ $line }};border-radius:18px;box-shadow:0 28px 70px rgba(18,38,63,.10);overflow:hidden;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:
            ...
            <tr><td align="center" style="padding:26px 26px 8px;">
              ...
              <h1 style="margin:14px 0 6px;color:{{ $ink }};font-size:22px;line-height:1.25;">
                Confirma tu correo, {{ $user->name }}
              </h1>
              <p style="margin:0 auto 4px;max-width:520px;color:{{ $muted }};line-height:1.65;">
                Gracias por registrarte. Para mantener tu cuenta segura, verifica tu dirección de correo.
              </p>
            </td></tr>
          </table>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
            <tr><td align="center" style="padding:20px 26px 6px;">
              ...
              <div style="word-break:break-all;color:{{ $brandInk }};font-size:12px;margin:8px 0 0;">
                {{ $url }}
              </div>

              <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin:18px 0 8px;">
                <tr>
                  <td style="padding:12px 14px;border:1px dashed {{ $line }};border-radius:12px;background:#fbfcff;color:{{ $muted }};font-size:13px;">
                    <strong style="color:{{ $ink }}">Consejo:</strong> El enlace expira en 60 minutos por seguridad.
                  </td>
                </tr>
              </table>
            </td></tr>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
            <tr><td align="center" style="color:#8a93a6;font-size:12px;padding:8px 26px 20px;">
              Este mensaje fue enviado automáticamente. Si no creaste esta cuenta, ignora este correo.
            </td></tr>
          </table>
        </td></tr>
